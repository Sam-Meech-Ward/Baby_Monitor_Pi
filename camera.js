...
module.exports.start = function(rtmpURL) {
  ...
  // const ffmpeg = spawn('ffmpeg', ['-i tcp://127.0.0.1:8181?listen', '-c:v libx264', '-preset ultrafast', '-tune zerolatency', '-an', '-c:v copy', '-c:a aac', '-ar 44100', '-f flv rtmp://206.189.218.99/myapp/pi']);
  // const ffmpeg = spawn('ffmpeg', ['-i', 'tcp://127.0.0.1:8181?listen']);
  // const ffmpeg = spawn('ffmpeg', ['-i', 'tcp://127.0.0.1:8181?listen', '-c:v', 'libx264', '-preset', 'ultrafast', '-tune', 'zerolatency', '-an', '-c:v', 'copy', '-c:a', 'aac', '-ar', '44100', '-f', 'flv rtmp://206.189.218.99/myapp/pi']);
